...
import com.sts.ssms.custom.calendar.helpers.LOW_ALPHA
import com.sts.ssms.custom.calendar.helpers.MEDIUM_ALPHA
import com.sts.ssms.custom.calendar.helpers.seconds
...
import com.sts.ssms.utils.adjustAlpha
import com.sts.ssms.utils.getContrastColor
import org.joda.time.DateTime
import org.joda.time.Days
import kotlin.random.Random

private const val BG_CORNER_RADIUS = 4f
...
class MonthView(context: Context, attrs: AttributeSet, defStyle: Int) :
    ...
    private fun groupAllEvents() {
        days.forEach {
            ...
            day.dayEvents.forEach { event ->
                ...
                val lastEvent =
                    allEvents.lastOrNull { monthViewEvent -> monthViewEvent.id == event.id }
                ...
                if ((lastEvent == null || lastEvent.startDayIndex + daysCnt <= day.indexOnMonthView) && !validDayEvent) {
                    val monthViewEvent = MonthViewEvent(
                        ...
                        event.title,
                        event.startTS,
                        ContextCompat.getColor(context, getRandomColors()),
        ...
        for (y in 0 until ROW_COUNT) {
            for (x in 0..6) {
                ...
                if (day != null) {
                    ...
                    val yPos = y * dayHeight + verticalOffset + 2
                    val xPosCenter = xPos + dayWidth / 2
                    if (day.isToday) {
                        canvas.drawCircle(
                            xPosCenter,
                            yPos + paint.textSize * 0.65f,
    ...
    private fun drawEvent(event: MonthViewEvent, canvas: Canvas) {
        ...
        for (i in 0 until event.daysCnt.coerceAtMost(7 - event.startDayIndex % 7)) {
            verticalOffset =
                verticalOffset.coerceAtLeast(dayVerticalOffsets[event.startDayIndex + i]) + 2
        }
...
//        val xPosCenter = xPos + dayWidth / 2
        ...
        val endDayIndex = days[(event.startDayIndex + event.daysCnt - 1).coerceAtMost(41)]
    ...
    private fun drawEventTitle(
        event: MonthViewEvent, canvas: Canvas, x: Float, y: Float,
        availableWidth: Float, startDay: DayMonthly, endDay: DayMonthly
    ) {
    ...
    private fun getEventTitlePaint(
        event: MonthViewEvent, startDay: DayMonthly, endDay: DayMonthly
    ): Paint {
    ...
    private fun initWeekDayLetters() {
        dayLetters = arrayListOf("M", "T", "W", "T", "F", "S", "S")
...
@ColorRes
fun getRandomColors(): Int = EVENT_COLORS[Random.nextInt(EVENT_COLORS.size)]
