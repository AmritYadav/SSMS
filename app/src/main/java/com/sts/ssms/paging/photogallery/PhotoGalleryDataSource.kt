...
class PhotoGalleryDataSource(private val galleryRepository: GalleryRepository) :
    PageKeyedDataSource<Int, AlbumUiModel>() {
    ...
    override fun loadInitial(
        ...
        callback: LoadInitialCallback<Int, AlbumUiModel>
    ) {
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, AlbumUiModel>) {
    ...
    override fun loadBefore(params: LoadParams<Int>, callback: LoadCallback<Int, AlbumUiModel>) {
    }

    private fun executeQuery(
        isInitialLoad: Boolean,
        page: Int,
        callback: (List<AlbumUiModel>) -> Unit
    ) {
        if (isInitialLoad) _initialLoad.postValue(NetworkState.LOADING)
        networkState.postValue(NetworkState.LOADING)
        coroutineScope.launch {
            val response = galleryRepository.albumList(page)
            if (response.albumList != null) {
                callback(response.albumList)
